...
const USER_COLUMNS = [
  ...
  { label: 'Roll Number', accessor: 'roll_number' },
  { label: 'Registration Number', accessor: 'registration_number' },
  { label: 'Registered At', accessor: 'created_at' },
];

const QUANTUM_COLUMNS = [
...
const AdminDashboard = () => {
  ...
  const handleExportUsers = useCallback(() => {
    setExporting(true);
    try {
      exportToExcel(filteredUsers, USER_COLUMNS, 'registered_users');
    } catch (err) {
  ...
  const handleExportQuantum = useCallback(() => {
    setExporting(true);
    try {
      exportToExcel(filteredQuantum, QUANTUM_COLUMNS, 'quantum_registrations');
    } catch (err) {
      console.error('Export error:', err);
      alert('Export failed. Please try again.');
    }
        ...
        <div style={{ display: 'flex', gap: '15px', alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <div className="admin-export-group">
            <button
              ...
              disabled={exporting}
              title="Export all data as CSV/Excel"
            >
              📥 {exporting ? 'Exporting…' : 'Export All'}
            </button>
            {activeTab === 'overview' && (
              <button className="admin-export-btn admin-export-btn-secondary" onClick={handleExportUsers} disabled={exporting}>
                📋 Export Users
              </button>
            )}
            {activeTab === 'quantum' && (
              <button className="admin-export-btn admin-export-btn-secondary" onClick={handleExportQuantum} disabled={exporting}>
                📋 Export Quantum
              </button>
            )}
          </div>
